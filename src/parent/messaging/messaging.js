...
class Messaging extends Component {
  ...
  constructor(props) {
    ...
    this.socket.on('connect', function() {
      ...
      self.socket.emit('adduser', user ? user.userId : null);
    });
    ...
    this.socket.on('updateusers', function(data) {     
      ...
      } else self.setOnlineUser(data, self.state.memberList, 'setOnlinePartnerStatus');
    });
  ...
  componentDidMount() {
    if (
      ...
      this.setState({
        ...
        shareProfileSendButton: true,
        profilePicture
      });
    }
    this.getUserAchievementInfo(this.props.shareUser.userId);
  }
      ...
      .then(response => {
        if (response.data.status === 'Success') {
          ...
          if (achievementData.length > 0) {
            ...
            this.shareProfile(competencyList);
          }
        }
      })
      .catch(err => {
  ...
  shareProfile(competencyList) {
    let _this = this;
    let sharedType = 'Message';
    let profileOwner = this.props.shareUser.userId
      ? this.props.shareUser.userId
      : this.props.shareUser.userId;
    let shareTime = moment().valueOf();
    //let profileName = this.state.profileName;
    let isActive = this.props.shareUser.isActive;
    let competencyData = competencyList;
    ...
    if (value.length > 0) {
      ...
      if (userId) {
        ...
        if (memberList && index !== -1) {
          ...
          let firstName = this.props.shareUser.firstName;
          let lastName = this.props.shareUser.lastName;
          let profilePicture = this.props.shareUser.profilePicture;
          let text =
  ...
  getOnlineUserStatus(memberList) {
    let data = {};
    if (this.state.partnerUserId) {
      let indexUser = memberList.findIndex(
        todo => todo.partnerId == parseInt(this.state.partnerUserId, 10)
      );
      data = memberList[indexUser];
    } else data = memberList[0];

    if (
      this.state.partnerUserId == data.partnerId ||
      !this.state.partnerUserId
    ) {
      this.setState({
        status: data.status
      });
  ...
  setOnlineUser(data, memberList, setStatus) {
    memberList =
      memberList &&
    ...
    if (setStatus == 'setOnlinePartnerStatus' && memberList)
      this.getOnlineUserStatus(memberList);
  ...
  showChatWindow = data => {
    if (this.state.shareProfileFlag && this.state.message !== '') {
      confirmAlert({
        customUI: ({ onClose }) => {
          return (
            <div className="custom-ui">
              {/* <h1>Are you sure?</h1> */}
              <p>Are you sure you want to discard this message ?</p>
              <button
                onClick={() => {
                  this.showChatWindowConfirm(data);
                  onClose();
                }}
              >
                Discard
  ...
  memberListMessageUpdate(data) {
    ...
    if (memberList && index !== -1) memberList[index]['lastMessage'] = null;
    this.setState({ memberList });
  }
  ...
  handleSendMessage() {
    ...
    if (shareProfile.length > 0 && this.state.shareProfileFlag) {
      ...
      for (let i = 0; i < shareProfile.length; i++) {
        ...
        let index =
          memberList &&
          memberList.findIndex(key => key.partnerId === parseInt(receiver, 10));
        if(index !== -1){  
          let sendShowChatDataa= memberList[index];
          _this.showShareChatWindow(sendShowChatDataa);
        }      
      }
      ...
      _this.sendMessageSubmit(messageObject);
      messageObject.fromMe = true;
                    ...
                    <InputGroup>
                      ...
                      <a
                        className={`flex align-center action--send rounded ${
                          this.state.message
                            ? !this.state.shareProfileSendButton
                              ? ''
                              : 'btn disabled'
                            : 'btn disabled'
                        }`}
...
const mapStateToProps = state => {
  return {
    user: state.User.parentData,
    shareUser: state.User.userData
  };
};
