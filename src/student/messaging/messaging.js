...
import { getThumbImage, limitCharacter,getAPIURL } from '../../common/commonFunctions';
...
class Messaging extends Component {
  ...
  constructor(props) {
    ...
    this.socket.on('connect', function() {
      ...
      let userData= {userId: user.userId, deviceId: getAPIURL().APIURL};
      self.socket.emit('adduser', userData);
    });
    ...
    this.socket.on('updateusers', function(data) {     
      if (!self.state.sessionStart) {      
        ...
        self.setOnlineUser(
          data,
          self.state.memberList,
          'setOnlinePartnerStatus'
        );
      }
    });
  }
  ...
  setOnlineUser(data, memberList, setStatus) {
    ...
    this.setState({ memberList,memberListFilter:memberList });
  }
  ...
  handleSendMessage() {
    ...
    if (shareProfile.length > 0 && this.state.shareProfileFlag) {
      ...
      for (let i = 0; i < shareProfile.length; i++) {
        ...
        let memberList= this.state.memberListFilter;   
        let index =
          memberList &&
